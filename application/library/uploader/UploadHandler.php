 ...
 * 
 * Small updates + polish languages support by lukasz@huncwot.com
 * 
 */
...
class UploadHandler
{
    ...
    protected $error_messages = array(
        'pl' =>
        [
            1 => 'Plik jest zbyt duży',
            2 => 'Plik jest zbyt duży (MAX_FILE_SIZE)',
            3 => 'Plik został wczytany fragmentarycznie',
            4 => 'Plik nie został załadowany',
            6 => 'Brak folderu tymczasowego',
            7 => 'Błąd zapisu pliku',
            8 => 'Rozszerzenie zatrzymało ładowanie pliku',
            'post_max_size' => 'Plik jest zbyt duży (#1)',
            'max_file_size' => 'Plik jest zbyt duży (#2)',
            'min_file_size' => 'Plik jest za mały (#3)',
            'accept_file_types' => 'Typ pliku nie jest obsługiwany',
            'max_number_of_files' => 'Zbyt duża ilość plików',
            'invalid_file_type' => 'Niepoprawny typ pliku',
            'max_width' => 'Obrazek jest zbyt szeroki',
            'min_width' => 'Obrazek jest zbyt wąski',
            'max_height' => 'Obrazek jest zbyt wysoki',
            'min_height' => 'Obrazek jest zbyt niski',
            'abort' => 'Wczytywanie pliku zatrzymane',
            'image_resize' => 'Błąd zmniejszania obrazku'
        ],
        'en' =>
        [
            1 => 'This file is too large',
            2 => 'This file is too large (MAX_FILE_SIZE)',
            3 => 'This file has been partly uploaded',
            4 => 'This file has not been uploaded',
            6 => 'Temporary folder not found',
            7 => 'File writing error',
            8 => 'Extension halted file upload',
            'post_max_size' => 'This file is too large (change post_max_size)',
            'max_file_size' => 'This file is too large (change max_file_size)',
            'min_file_size' => 'This file is too small (change min_file_size)',
            'accept_file_types' => 'This extension is not supported',
            'max_number_of_files' => 'Too many files uploaded (max_number_of_files)',
            'invalid_file_type' => 'Invalid file type',
            'max_width' => 'Image too wide',
            'min_width' => 'Image too narrow',
            'max_height' => 'Image too high',
            'min_height' => 'Image too low',
            'abort' => 'Upload aborted',
            'image_resize' => 'Image resize error'
        ]
    );
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null, $lang = 'en')
    {
        $this->options = array(
        ...
        $this->error_messages = $this->error_messages[$lang];
    ...
    protected function get_download_url($file_name, $version = null, $direct = false, $path = false)
    {
    ...
    protected function get_file_object($file_name)
    {
        if ($this->is_valid_file_object($file_name)) {
            ...
            $file->path = $file->path = parse_url($file->url, PHP_URL_PATH);
    ...
    protected function gd_create_scaled_image($file_name, $version, $options)
    {
        ...
        if ($success) {
            if ($write_func == 'imagegif')
                $success = $write_func($new_img, $new_file_path);
            else $success = $write_func($new_img, $new_file_path, $image_quality);
        }
    ...
    protected function handle_file_upload(
        ...
        if ($this->validate($uploaded_file, $file, $error, $index, $content_range)) {
            ...
            if ($file_size === $file->size) {
                $file->url = $this->get_download_url($file->name);
                $file->path = parse_url($file->url, PHP_URL_PATH);
    ...
    {
        if (isset($_SERVER[$id])) return $_SERVER[$id];
        else return ('');
    }
    ...
    public function generate_response($content, $print_response = true)
    {
        ...
        if ($print_response) {
            ...
            // ckeditor
            if (@$this->options['response_type'] == 'ckeditor') {
                $content =
                    [
                        "uploaded" => 1,
                        "fileName" => $content['upload'][0]->name,
                        "url" => $content['upload'][0]->path
                    ];
            }
            ...
            if (empty($content['files']))
                $content['files'] = [
                    [
                        "error" => 'No files uploaded. Check server limits.'
                    ]
                ];

            $json = json_encode($content);
            $redirect = $this->get_post_param('redirect');
            if ($redirect) $redirect = stripslashes($redirect);
